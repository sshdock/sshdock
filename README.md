...
- `sshdock config set/import/list/keys/get/unset` stores app-scoped config over SSH; list output is redacted, keys output is script-friendly, get is an explicit reveal, and values are encrypted in SQLite with a host-local key outside the database.
