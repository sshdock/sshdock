...
Replace `v0.2.1` with the release tag you want to install. Replace `example.com` with a real base domain. Point `sshdock.example.com` and wildcard app DNS such as `*.example.com` at the server before expecting public Git, HTTP, or HTTPS traffic to work.

For runnable confidence checks after installation, see [`EXAMPLES.md`](EXAMPLES.md). It includes a minimal static site and a lite WordPress stack that can be fetched into a new local Git repository and pushed through SSHDock.
...
```bash
sudo SSHDOCK_TAG=v0.2.1 SSHDOCK_BOOTSTRAP_INSTALL_DEPS=0 bash bootstrap.sh
```
